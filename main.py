        ...
        try:
            ...
            for datum in data:
                ...
                if datum[0] == str(day):
                    # print(datum[0])
                    if (cursor.execute(
                            f"UPDATE Sales_{month} SET Total_Kg='" + str(monthly_kg) + "', Total_Amount='" + str(
                                    monthly_total) + "' WHERE Date='" + str(day) + "'")):
                        cursor.execute('commit')
                        # print("Successfully")
                        break
                    else:
                ...
                fill = f"INSERT INTO Sales_{month} VALUES('" + str(day) + "', '" + str(monthly_kg) + "', '" + str(
                    monthly_total) + "')"
        ...
        try:
            ...
            for datum in data:
                ...
                if datum[0] == str(month):
                    # print(datum[0])
                    if (cursor.execute(
                            f"UPDATE Trans_{year} SET Total_Kg='" + str(total_monthly_kg) + "', Total_Exp='" + str(
                                    expenses_) + "', Total_Amount='" + str(
                ...
                fill = f"INSERT INTO Trans_{year} VALUES('" + str(month) + "', '" + str(
                    total_monthly_kg) + "', '" + str(expenses_) + "', '" + str(total_monthly_sales) + "')"
                if (cursor.execute(fill)):
                    cursor.execute('commit')
                    # print("Successfully")
                else:
